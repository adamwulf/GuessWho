...
- **Extension file log** — during the scroll pass, `content.js` sends bounded
  ...
  probe id, elapsed time, readiness transition, scroller dimensions, section
  headings, and an attribute/count fingerprint of the Experience heading's
  ancestors. It intentionally excludes full HTML, URL query strings, cookies,
  and photo bytes. The native side caps every serialized record at 32,768
  characters even if a malformed sender bypasses the JS bounds.
