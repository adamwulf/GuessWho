...
**Status:** APPROVED by both reference agents; all refinements folded in. Distribution ANSWERED (App Store + TestFlight + Setapp; mirror Allume/Muse). All three MAS unknowns RESOLVED — Muse ships this seam in production (symlink works on MAS via the runtime auth API; client launches the helper which connects to the app; App Review clean). Symlink-entitlement question CLOSED (needs no special key — just app-sandbox + the runtime auth API). Phase 0 is a confirmation, not a discovery. **NO open items remain** — ready for Phase 0 on Adam's go-ahead. · **Date:** 2026-07-16
