...
5. **`/usr/local/bin` symlink install WORKS on MAS (Adam-confirmed for Muse) via a specific entitlement** — install via `NSWorkspace.requestAuthorization(.createSymbolicLink)` + `FileManager(authorization:)`, resolves back to the running bundle through the 4-state resolver. Confirm our MAS build carries **the same entitlement Muse uses** (name being obtained from allume). The symlink is therefore a shipping install option on ALL channels; copy-path remains the always-works fallback.
...
1. **MAS symlink install WORKS** — there is an **entitlement that allows it** on the MAS build. So the `/usr/local/bin` symlink is a real, shipping option on the App Store channel, not Setapp-only. (Exact entitlement name being confirmed with allume — fold in.) The 4-state resolver + copy-path button still ship as the robust baseline, but the symlink is proven on MAS.
...
**What this changes for Phase 0:** the packaging seam is no longer an open risk — it is **proven in production by Muse**, and GuessWho mirrors it. Phase 0 becomes a *confirmation that our build reproduces Muse's working setup* (esp. the per-channel app-group derivation + the symlink entitlement), not a discovery of whether the approach is viable. The symlink can be the primary install on all channels once we carry Muse's entitlement; copy-path remains the always-works fallback. **Action:** get the exact symlink-enabling entitlement + Muse's Info.plist/bundle wiring from allume (Muse is in the store and working — mirror it).
