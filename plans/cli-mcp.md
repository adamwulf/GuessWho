...
**⚠️ THREE EMPIRICAL MAS-REVIEW / MAS-RUNTIME UNKNOWNS — a plan-level GATE risk, only Adam can answer (allume verified the CODE is written to work on the sandboxed build, but NOT that it passes review / runs in production — do not encode inference as fact):**
1. **MAS symlink install:** does the sandbox-legal `NSWorkspace.requestAuthorization(.createSymbolicLink)` + `FileManager(authorization:)` install actually SUCCEED on the shipped MAS build, or does MAS fall back to the copy-path/manual-config UX in practice? (Muse's installer shows a manual `ln -s`/`rm` fallback in the same pane — allume's *inference* is that the copy-path flow is the real always-works path on MAS, symlink best-effort — but that is inference, not verified shipped behavior.)
2. **MAS client-spawn:** does an MCP client successfully exec `.../GuessWho.app/Contents/MacOS/guesswho` from a **Mac-App-Store-installed** app (Gatekeeper + receipt validation), and any translocation/quarantine gotcha vs the Setapp copy?
3. **App Review pushback:** did Apple (or Setapp) push back on the embedded CLI helper, named pipes, or the MCP concept at all?

**Why it's a GATE, not an implementation detail:** if MAS refuses the embed or the spawn, the whole install story becomes manual-config-only on the App Store channel — a plan-level decision. **Get Adam's production answers BEFORE committing Phase 0's install UX to the symlink.** Design default until answered: make **copy-the-`Contents/MacOS`-path-into-`.mcp.json`** the PRIMARY, always-works install flow on every channel; treat the `/usr/local/bin` symlink as a best-effort nicety where the sandbox permits (Setapp/Developer-ID).

**NEITHER reference impl has proven MAS-review survival (confirmed 2026-07-16):** EssentialMCP is prior art for the CODE PATTERN only — it is NOT a shipped sandboxed MAS app (its entitlements lack `app-sandbox`, the helper still carries `get-task-allow`, and there is zero symlink/receipt code), so it validates nothing about App Review or MAS runtime. Muse (via allume) is the closest empirical source but allume correctly declined to assert its shipped MAS behavior from code alone. **Therefore MAS-exec-of-the-embedded-helper is UNPROVEN and stays inside the Phase 0 gate:** explicitly test client-exec from a real sandboxed MAS/TestFlight build, and don't rely on the symlink auth API on MAS until verified on that build.
