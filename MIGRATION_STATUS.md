...
| 5 | _SHAs filled post-merge_ | iPhone UIKit shell (`PermissionGateViewController` + `UITabBarController` sharing Catalyst's list VCs); deleted `RootView.swift` + 4 SwiftUI list views + `View.contactAndEventDestinations`; AppDelegate owns `eventsRepository` and the `.CNContactStoreDidChange`/`.EKEventStoreChanged` observers (single fan-out point); SwiftUI `NavigationLink(value:)` bridged to outer UIKit nav via env-injected `pushContactReference`/`pushEventReference` closures (5 callsites). |
